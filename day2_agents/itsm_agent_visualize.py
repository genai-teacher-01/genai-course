...
def build_langgraph_agent():
    ...
    def route_after_risk_check(state: AgentState) -> Literal["human_approval", "agent"]:
...
def cmd_visualize(args: argparse.Namespace) -> None:
    ...
    mermaid_path = output_dir / "itsm_graph.mmd"
    mermaid_path.write_text(mermaid, encoding="utf-8")

    print(f"File Mermaid salvato in: {mermaid_path}")

    try:
        png = graph_view.draw_mermaid_png()
        png_path = output_dir / "itsm_graph.png"
        png_path.write_bytes(png)
        print(f"PNG salvato in: {png_path}")

    except Exception as exc:
        print(
            "Non sono riuscito a generare il PNG. "
            "Il file .mmd è stato comunque salvato correttamente."
        )
