...
import com.genai.course.multiagent.model.TicketRecord;
...
public class TicketStoreService {
    ...
    static {
        TICKETS.put("INC-1002", new TicketRecord(
                ...
                false, List.of("email", "finance", "production", "p1"), List.of("CHG-2201"),
                List.of(new TicketRecord.Comment("finance.ops", "Impatto confermato su più uffici.", "2026-05-11T09:20:00"),
                        new TicketRecord.Comment("team-sre", "Verifica in corso su mail gateway e bilanciatore.", "2026-05-11T09:42:00"))));
        TICKETS.put("INC-1003", new TicketRecord(
                ...
                true, List.of("api", "procurement", "latency", "p2"), List.of(),
                List.of(new TicketRecord.Comment("s2p.business", "Workaround scomodo ma utilizzabile.", "2026-05-11T10:15:00"))));
        TICKETS.put("INC-1004", new TicketRecord(
                ...
                true, List.of("access-request", "sales", "p3"), List.of(), List.of()));
        TICKETS.put("INC-1005", new TicketRecord(
                ...
                false, List.of("mainframe", "billing", "abend", "p1"), List.of("PRB-778"),
                List.of(new TicketRecord.Comment("batch.monitoring", "Codice ABEND rilevato: S0C7.", "2026-05-11T06:10:00"))));
    }

    public ToolResponse lookupRecord(String recordId) {
        if (recordId == null || recordId.isBlank()) return ToolResponse.fail("record_id vuoto.");
        TicketRecord ticket = TICKETS.get(recordId.toUpperCase().trim());
        if (ticket == null) return ToolResponse.fail("Record non trovato: " + recordId);
