...
- server MCP SDK v2 reale, compatibile con il protocollo MCP 2026-07-28;
- client moderno con negoziazione automatica server/discover e fallback legacy;
- backend MCP reale selezionabile per l'agente sincrono del laboratorio.
...
Architettura:
        ...
        +---------+---------+
        v                   v
     MCPAdapter         MCPServer v2           (un solo lato attivo)
...
Comandi:
    python day3_multiagent/mcp_lab_v2.py examples
    python day3_multiagent/mcp_lab_v2.py list-tools
    python day3_multiagent/mcp_lab_v2.py describe search_kb
    python day3_multiagent/mcp_lab_v2.py call search_kb '{"query":"policy P1","top_k":2}'
    python day3_multiagent/mcp_lab_v2.py agent "Mostrami INC-1002 e cerca la policy P1"
    python day3_multiagent/mcp_lab_v2.py agent "Mostrami INC-1002 e cerca la policy P1" --fast
    python day3_multiagent/mcp_lab_v2.py agent "Mostrami INC-1002 e cerca la policy P1" --verbose
    python day3_multiagent/mcp_lab_v2.py cost-from-run runs/day3_mcp/agent_YYYYMMDD_HHMMSS_xxxxxxxx.json
    python day3_multiagent/mcp_lab_v2.py serve
    python day3_multiagent/mcp_lab_v2.py inspector
...
Variabili .env:
    ...
    MCP_BACKEND=mcp                # "mcp" (default) | "adapter"
    MCP_SERVER_CMD="python day3_multiagent/mcp_lab_v2.py serve"
...
MCP_BACKEND = os.getenv("MCP_BACKEND", "mcp").lower().strip()
...
# ---------------------------------------------------------------------
# Server MCP SDK v2 — protocollo MCP 2026-07-28
# ---------------------------------------------------------------------

def build_mcp_server() -> Any:
    """Costruisce il server MCP reale con tool, resource e prompt ITSM."""
    try:
        from mcp.server import MCPServer
    except Exception as exc:
        raise RuntimeError(
            "MCP Python SDK v2 non installato. Installa con: "
    ...
    mcp = MCPServer(
        ...
        version="2.0.0",
        instructions=(
            "Server didattico ITSM: usa lookup_record per i ticket, "
            "compute_sla per lo stato SLA e search_kb per policy e procedure."
        ),
    )
    ...
    @mcp.resource(
        "itsm://playbooks/p1-escalation",
        mime_type="text/markdown",
    )
    def p1_escalation_playbook() -> str:
        """Playbook Markdown per incident P1 ed escalation ITSM."""
        path = BASE_DIR / "resources" / "p1_escalation_playbook.md"
        return path.read_text(encoding="utf-8")

    @mcp.prompt()
    def p1_incident_briefing(
        ticket_id: str,
        audience: str = "service_manager",
        language: str = "italiano",
    ) -> str:
        """Prompt riusabile per produrre un briefing strutturato su un incident P1."""
        return f"""
                Sei un Incident Commander ITSM.

                Obiettivo:
                prepara un briefing operativo in {language} per il destinatario: {audience}.

                Ticket da analizzare:
                {ticket_id}

                Procedura obbligatoria:
                1. Usa lookup_record per recuperare i dati del ticket.
                2. Se il ticket esiste, usa compute_sla con priorità, elapsed_hours, owner, service e workaround_available.
                3. Usa search_kb per cercare policy P1, SLA, escalation e azioni critiche.
                4. Non inventare dati mancanti: dichiara esplicitamente ciò che non è disponibile.
                5. Distingui tra:
                - escalation tecnica;
                ...
                - Executive summary, massimo 5 righe.
                - Dati ticket rilevanti.
                - Stato SLA e motivazione.
                - Evidenze documentali con fonte.
                - Rischio business.
                - Raccomandazione operativa.
                - Azioni che richiedono approvazione umana.
                - Prossimo messaggio consigliato da inviare agli stakeholder.

                Tono:
                chiaro, operativo, non narrativo, adatto a una war-room ITSM.
                """.strip()
...
def run_mcp_server() -> None:
    """Avvia il server MCP v2 su stdio."""
...
def run_mcp_http_server(
    ...
    """Avvia il server MCP v2 su Streamable HTTP."""
...
def sdk_model_to_wire(value: Any) -> Any:
    """Converte un modello MCP/Pydantic nei nomi JSON usati sul wire."""
    if value is None:
        return None
...
def serialize_sdk_tool_result(result: Any) -> Dict[str, Any]:
    """Normalizza CallToolResult v2 nel formato wire-style usato dal lab."""
    return {
        "content": [sdk_model_to_wire(block) for block in (result.content or [])],
        "structuredContent": getattr(result, "structured_content", None),
        "isError": bool(getattr(result, "is_error", False)),
    }


async def mcp_discovery(command: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """Avvia un server stdio e scopre capability e tool con il Client v2."""
    try:
        from mcp import Client, StdioServerParameters
        from mcp.client.stdio import stdio_client
    except Exception as exc:
    ...
    if not argv:
        raise RuntimeError("MCP_SERVER_CMD è vuoto.")
    ...
    async with Client(stdio_client(params)) as client:
        ...
        return tools, {
            "server": sdk_model_to_wire(client.server_info),
            "protocol_version": str(client.protocol_version),
            "capabilities": sdk_model_to_wire(client.server_capabilities),
        }
...
async def http_mcp_call_tool(
    ...
    """Chiama un tool remoto; Client v2 seleziona Streamable HTTP dalla URL."""
    try:
    ...
    async with Client(url) as client:
        ...
        if tool_name not in available_tools:
            return {
                "url": url,
                "server_info": sdk_model_to_wire(client.server_info),
                "protocol_version": str(client.protocol_version),
        ...
        return {
            ...
            "tool": tool_name,
            "args": args,
            "mcp_result": serialize_sdk_tool_result(result),
        }
...
async def mcp_call_tool(
    ...
    """Avvia un server stdio e chiama un tool mediante il Client MCP v2."""
    try:
        from mcp import Client, StdioServerParameters
        from mcp.client.stdio import stdio_client
    except Exception as exc:
...
class MCPStdioAdapter(MCPAdapter):
    """
    Ponte sincrono per il lab: ogni operazione usa un vero Client MCP v2 stdio.

    L'apertura di un subprocess per chiamata privilegia semplicità e isolamento
    didattico. In produzione conviene mantenere un Client asincrono persistente.
    """

    def __init__(self, command: str):
        super().__init__(name="hcl-itsm-mcp-v2-client")
        self.command = command
        self._discovered_tools: Optional[List[Dict[str, Any]]] = None

    def list_tools(self) -> List[Dict[str, Any]]:
        if self._discovered_tools is None:
            tools, _ = asyncio.run(mcp_discovery(self.command))
    ...
    def call_tool(self, name: str, **kwargs: Any) -> Dict[str, Any]:
        response = asyncio.run(mcp_call_tool(self.command, name, kwargs))
        if "mcp_result" in response:
            return response["mcp_result"]
        return model_to_dict(
            MCPToolResult(
                content=[{
                    "type": "text",
                    "text": response.get("error", f"Tool '{name}' non disponibile."),
                }],
                isError=True,
            )
        )
...
def build_mcp_client() -> MCPAdapter:
    """Seleziona adapter in-process oppure vero client MCP v2 su stdio."""
    if MCP_BACKEND == "adapter":
        return build_adapter()
    if MCP_BACKEND == "mcp":
        return MCPStdioAdapter(MCP_SERVER_CMD)
    raise RuntimeError(
        f"MCP_BACKEND={MCP_BACKEND!r} non valido; usa 'adapter' oppure 'mcp'."
    )
...
EXAMPLE_PROMPTS: List[Dict[str, str]] = [
    ...
    },
    {
        ...
        "prompt": (
            "python day3_multiagent/mcp_lab_v2.py agent "
    ...
    },
    {
        ...
        "prompt": (
            "python day3_multiagent/mcp_lab_v2.py cost-from-run "
            "runs/day3_mcp/agent_YYYYMMDD_HHMMSS_xxxxxxxx.json"
        ),
    },
    {
        "title": "7. Inspector MCP v2 reale",
        "peculiarity": "Apre un MCPServer v2 e negozia server/discover + tools/list.",
        "prompt": "python day3_multiagent/mcp_lab_v2.py inspector",
    },
]
...
def cmd_serve(_: argparse.Namespace) -> None:
    """Avvia il server MCP v2 reale su stdio."""
    run_mcp_server()


def cmd_serve_http(args: argparse.Namespace) -> None:
    """
    Avvia il server MCP v2 reale su Streamable HTTP.
...
def cmd_inspector(args: argparse.Namespace) -> None:
    """
    Inspector minimale: apre `MCP_SERVER_CMD`, negozia la versione e lista i tool.
    """
    command = args.command or MCP_SERVER_CMD
    print(f"Inspector → comando server: {command}")

    try:
        tools, info = asyncio.run(mcp_discovery(command))
...
def cmd_inspector_call(args: argparse.Namespace) -> None:
    """
    Inspector avanzato: apre `MCP_SERVER_CMD` e chiama realmente un tool MCP.
    """
    command = args.command or MCP_SERVER_CMD
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Espone tool ITSM tramite adapter simulato o MCPServer v2 reale, "
